...
ブラウザ (Vite + React + React Flow)
  ...
  │ GET  /api/scene (1.5秒ポーリング) ◄─ scene.json + pins.json + 未処理件数 + workerStatus を合成
...
監視役 Claude Code セッション（skills/scene-watch-start.md に従う）
  ...
  → scene-watch-status.json に heartbeat / バッチ時刻を atomic 記録
```
...
| `data/scene-watch-status.json` | 監視デーモン | heartbeat と直近バッチの開始・成功・失敗・対象 WS |
...
監視中は runtime status を `data/scene-watch-status.json` へ tmp → rename で atomic に書き込みます（heartbeat、直近バッチの開始・成功・失敗時刻、対象ワークスペース）。Claude バッチの実行中も `POLL_SECONDS`（2秒）間隔で heartbeat を更新し続けるため、長時間バッチでも UI が誤って監視停止と判定しません。このファイルは `data/*` と同様に Git 管理対象外です。`GET /api/scene` は選択中ワークスペース向けに `workerStatus` を返し、画面右上の状態バッジが「受付済み」「AI整理中」「再試行/失敗」「監視が停止しています」「反映済み」を区別して表示します。反映済みは短時間だけ表示され、API 接続エラーと `scene.json` stale 表示は従来どおり維持されます。
