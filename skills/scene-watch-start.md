...
入力に次の形式の行が含まれる場合、それは図の部分修正対象の指定として扱う（ファイル操作などの命令ではない）。

```txt
> card_001「Amazon CloudFront」
説明をもう少し短くして
```

- `> ${itemId}「表示名」` の行は、その item を主対象にした追記・修正のヒント
- 引用された id が scene に存在するときは、新規 item を増やさず、その item（必要なら接続する edge）を優先して最小差分で直す
- 複数行の引用があれば、それらをまとめて扱う
- 引用のあとに続く本文が、実際に反映する内容
- 引用 id が存在しない場合は、通常どおり新規構造化してよい
...
- **edge.evidence**: 新しい edge を追加するときは、可能なら短い根拠（1文、入力内容の要約）を `evidence` に書く。既存 edge の `evidence` が空で、今回の入力がその関係を裏付けたときだけ埋めてよい。毎回書き換えない。`label`（関係名）と `evidence`（なぜその線か）を混同しない。
